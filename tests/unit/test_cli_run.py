...
from src.cli.run import run_case, run_path
...
def test_run_path_executes_all_yaml_cases_in_directory(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_run_case(path: Path) -> bool:
        executed.append(path.name)
        return True

    monkeypatch.setattr("src.cli.run.run_case", fake_run_case)
