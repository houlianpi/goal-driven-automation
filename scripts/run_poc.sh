...
SCENARIOS_FILE="$PROJECT_DIR/scenarios/poc_goals.json"
REPORT_DIR="$PROJECT_DIR/runs/poc-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$REPORT_DIR"
echo "POC Runner"
echo "Report: $REPORT_DIR"
cd "$PROJECT_DIR"
TOTAL=0; PASSED=0; FAILED=0; REPAIRED=0
python3 -c "import json; f=open('scenarios/poc_goals.json'); d=json.load(f); [print(s['id'],'|',s['goal']) for s in d['scenarios']]" | while IFS="|" read -r id goal; do
  TOTAL=$((TOTAL+1))
  echo "Running: $id"
